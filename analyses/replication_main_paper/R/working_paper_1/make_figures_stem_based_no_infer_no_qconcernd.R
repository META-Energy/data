...
# Remove qconcern and inferior observations ----
d <- d %>% filter(qconcernd == 0,
                  inferior_study == 0)
...
(stem_funnel_sr <- stem_funnel(d_stem_sr %>% pull(coeffc), 
                               ...
                               beta_axis_max = 0.5))
# Save as PDF
dev.copy(pdf, paste0(figure_path, "stem_based/", "no_infer_no_qconcernd/figure_stem_funnel_sr.pdf"), 
         width = 10, 
         height = 10)
dev.off()
...
(stem_funnel_lr <- stem_funnel(d_stem_lr %>% pull(coeffc), 
                               ...
                               beta_axis_max = 0.5))
# Save as PDF
dev.copy(pdf, paste0(figure_path, "stem_based/", "no_infer_no_qconcernd/figure_stem_funnel_lr.pdf"), 
         width = 10, 
         height = 10)
dev.off()
...
# Save as RData
save(stem_estimates, file = paste0(figure_path, "stem_based/", "no_infer_no_qconcernd/stem_estimates.RData"))
...
# Save as CSV
write.csv(stem_estimates, paste0(figure_path, "stem_based/", "no_infer_no_qconcernd/table_stem_estimates.csv"), row.names = FALSE)
