...
calculate_factor_changes <- function(results_df) {
  ...
  upper_bound <- as.numeric(gsub("[^0-9.-]", "", strsplit(last_threshold, ",")[[1]][2]))
  ...
  for(i in 1:n) {
    for(j in 1:n) {
      if(i != j) {
        factor_matrix[i,j] <- threshold_rows$estimate[j] / threshold_rows$estimate[i]
      }
    }
  }
...
add_confidence_bands <- function(current_plot, tidy_data, start_row = 4, 
                                 band_alpha = 0.2, band_color = "blue") {
...
wins_para <- 0.02  # winsorization
ap <- FALSE # Adequately powered (80% or more)
prec_weighted <- FALSE
cluster_se <- TRUE
AK_critvals <- c(# Cutoff for statistical significance
  1.96 # 95%
)
AK_symmetric <- FALSE
AK_modelmu <- "t"
...
## Results ----
### Estimation table ----
modelsummary::modelsummary(list("All horizons" = ak_results[[1]], "Short run" = ak_results_sr[[1]], "Long run" = ak_results_lr[[1]]),
                           output = "gt", 
                           stars = T,
...
#### Save as PNG
modelsummary::modelsummary(list("All horizons" = ak_results[[1]], "Short run" = ak_results_sr[[1]], "Long run" = ak_results_lr[[1]]),
                           output = paste0(figure_path, "publication_probability/AK estimation_cutoff_95.png"), 
                           stars = T,
                           statistic = "se: {std.error}",
                           conf_level = 0.89, 
                           title = paste0("AK estimation; cutoff: ", AK_critvals), 
...
print(
  xtable::xtable(factor_table_sr, caption = "Factor Changes Between Thresholds"), 
  type = "latex"
  )
#### Long run estimation ----
...
print(
  xtable::xtable(factor_table_lr, caption = "Factor Changes Between Thresholds"), 
  type = "latex"
  )
...
all_95_plot
ggsave(paste0(figure_path, "publication_probability/figure_publication_probability_all_95_log.pdf"), 
       plot = all_95_plot, device = "pdf", width = width, height = height)
# Short run
sr_95_plot <- add_confidence_bands(ak_results_sr[[1]]$plot, ak_results_sr[[1]]$tidy)
sr_95_plot
ggsave(paste0(figure_path, "publication_probability/figure_publication_probability_short_run_95_log.pdf"), 
       plot = sr_95_plot, device = "pdf", width = width, height = height)
# Long run
lr_95_plot <- add_confidence_bands(ak_results_lr[[1]]$plot, ak_results_lr[[1]]$tidy) 
lr_95_plot
ggsave(paste0(figure_path, "publication_probability/figure_publication_probability_long_run_95_log.pdf"), 
       plot = lr_95_plot, device = "pdf", width = width, height = height)

# Estimation for 1.645 (90 %), 1.96 (95 %), 2.576 (99 %) levels (for appendix) ----

## Define function to add confidence bands for multiple cutoffs ----
add_confidence_bands_multiple <- function(current_plot, tidy_data, cutoffs, start_row = 4, 
                                          band_alpha = 0.2, band_color = "blue") {
...
## Specifiy input arguments for estimation with meta_analysis() function ----
AK_critvals_all <- c(# Cutoffs for statistical significance
  1.645, # 90%
  1.96, # 95%
  2.576 # 99%
)
...
## Results ----
### Estimation table ----
modelsummary::modelsummary(list("All horizons" = ak_results_all_cutoffs[[1]], "Short run" = ak_results_sr_all_cutoffs[[1]], "Long run" = ak_results_lr_all_cutoffs[[1]]),
                           output = "gt", 
                           stars = T,
                           statistic = "se: {std.error}",
                           conf_level = 0.89, 
                           title = paste0("AK estimation; cutoffs: ", AK_critvals_all[1], ", ", AK_critvals_all[2], ", ", AK_critvals_all[3]),
                           gof_map = NULL)
#### Save as PNG
modelsummary::modelsummary(list("All horizons" = ak_results_all_cutoffs[[1]], "Short run" = ak_results_sr_all_cutoffs[[1]], "Long run" = ak_results_lr_all_cutoffs[[1]]),
                           output = paste0(figure_path, "publication_probability/AK estimation_cutoffs_90_95_99.png"), 
                           stars = T,
                           statistic = "se: {std.error}",
                           conf_level = 0.89, 
                           title = paste0("AK estimation; cutoffs: ", AK_critvals_all[1], ", ", AK_critvals_all[2], ", ", AK_critvals_all[3]),
...
### Calculat factors ----
#### Short run estimation ----
factor_table_sr_all_cutoffs <- calculate_factor_changes(ak_results_sr_all_cutoffs[[1]]$tidy)
print(factor_table_sr_all_cutoffs)
print(
  xtable::xtable(factor_table_sr_all_cutoffs, caption = "Factor Changes Between Thresholds"), 
  type = "latex"
  )
#### Long run estimation ----
factor_table_lr_all_cutoffs <- calculate_factor_changes(ak_results_lr_all_cutoffs[[1]]$tidy)
print(factor_table_lr_all_cutoffs)
print(
  xtable::xtable(factor_table_lr_all_cutoffs, caption = "Factor Changes Between Thresholds"), 
  type = "latex"
  )
...
width <- 5; height <- 3
# All horizons
all_multiple_plot <- add_confidence_bands_multiple(ak_results_all_cutoffs[[1]]$plot, 
                                                   ak_results_all_cutoffs[[1]]$tidy, 
                                                   AK_critvals_all)
all_multiple_plot
ggsave(paste0(figure_path, "publication_probability/figure_publication_probability_all_all_cutoffs_log.pdf"), 
       plot = all_multiple_plot, device = "pdf", width = width, height = height)

# Short run
sr_multiple_plot <- add_confidence_bands_multiple(ak_results_sr_all_cutoffs[[1]]$plot, 
                                                  ak_results_sr_all_cutoffs[[1]]$tidy, 
                                                  AK_critvals_all)
sr_multiple_plot <- sr_multiple_plot +
  # Adjust y-axis limits to show all CIs
  ggplot2::scale_y_log10(limits = c(0.3, 50))
sr_multiple_plot
ggsave(paste0(figure_path, "publication_probability/figure_publication_probability_short_run_all_cutoffs_log.pdf"), 
       plot = sr_multiple_plot, device = "pdf", width = width, height = height)

# Long run
lr_multiple_plot <- add_confidence_bands_multiple(ak_results_lr_all_cutoffs[[1]]$plot, 
                                                  ak_results_lr_all_cutoffs[[1]]$tidy, 
                                                  AK_critvals_all)
lr_multiple_plot <- lr_multiple_plot +
  # Adjust y-axis limits to show all CIs
  ggplot2::scale_y_log10(limits = c(0.1, 25))
lr_multiple_plot
ggsave(paste0(figure_path, "publication_probability/figure_publication_probability_long_run_all_cutoffs_log.pdf"), 
       plot = lr_multiple_plot, device = "pdf", width = width, height = height)
